...
class KeyboardEditingTest(unittest.TestCase):
    ...
    def test_move_and_undo_are_atomic_and_escape_preserves_selection(self):
        ...
        self.assertEqual(self.win.timeline.vclips[0].start, 1)
        self.key(Gdk.KEY_l)
        self.assertEqual(self.win.video_clips[0].start, 2)
        self.assertEqual(self.win.timeline.vclips[0].start, 2)
        self.assertEqual(len(self.win._history), 3)
    ...
    def test_source_audio_detaches_only_when_changed_and_undo_restores_link(self):
        ...
        self.assertEqual(self.win.timeline.aclips[0].start, 1)
